...
public:
    ...
    void ChangeFirstName(int year, const std::string& first_name) {
        if(!years_to_names.empty() && years_to_names.begin()->first > year)
            return;
        years_to_names[year] = first_name;
    }
    void ChangeLastName(int year, const std::string& last_name) {
        if(!years_to_lastnames.empty() && years_to_lastnames.begin()->first > year)
            return;
        years_to_lastnames[year] = last_name;
    }
...
private:
    ...
    std::string form_history(Iterator start, Iterator end)const {
        std::string history;
        bool first = true;
